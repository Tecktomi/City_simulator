		...
		if array_length(trabajos) > 0{
			...
			for(var a = persona.educacion; a >= 0; a--)
				if array_length(trabajo_educacion[a]) > 0{
					...
					if trabajo != persona.trabajo and trabajo.trabajo_calidad > persona.trabajo.trabajo_calidad / (1 + persona.trabajo.huelga) and (persona.religion or not edificio_es_iglesia[trabajo.tipo]) and ((persona.sexo and not persona.religion) or not (edificio_nombre[trabajo.tipo] = "Cabaret")){
