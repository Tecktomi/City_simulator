	...
	with control{
		...
		if var_edificio_nombre = "Mina"{
			...
			for(var a = max(0, edificio.x - 1); a < min(xsize - 1, edificio.x + width + 1); a++)
				for(var b = max(0, edificio.y - 1); b < min(xsize - 1, edificio.y + height + 1); b++)
			...
			var c = 0
			for(var a = max(0, edificio.x - 5); a < min(edificio.x + width + 5, xsize); a++)
				for(var b = max(0, edificio.y - 5); b < min(edificio.y + height + 5, ysize); b++)
					if bosque[a, b]
						c += bosque_madera[a, b]
