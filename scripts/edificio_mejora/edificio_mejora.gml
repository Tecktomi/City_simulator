function edificio_mejora(edificio = control.null_edificio, mejora = control.null_mejora){
	...
	with control{
		if not edificio.privado and (not agua or edificio.agua) and (not energia or edificio.energia){
			...
			if not array_contains(edificio.mejoras, string(nombre)) and floor(dia / 365) >= anno and draw_boton(room_width - 40, pos, $"{flag ? "" : "Desbloquear "}{nombre} ${precio}",,, function(text){draw_text(mouse_x, mouse_y, text)}, descripcion) and dinero >= precio + precio_tecnologia{
				if not flag{
